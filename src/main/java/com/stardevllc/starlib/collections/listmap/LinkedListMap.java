...
public class LinkedListMap<K, V> implements ListMap<K, V> {
    
    private final Map<K, LinkedList<V>> backingMap = new HashMap<>();
    
    @Override
    public int size() {
        int size = 0;
        for (LinkedList<V> list : this.backingMap.values()) {
            size += list.size();
        }
        return size;
    }
    ...
    public V get(K key, int index) {
        LinkedList<V> list = this.backingMap.get(key);
        if (list == null) {
            return null;
        }
    ...
    public void add(K key, V value) {
        this.backingMap.computeIfAbsent(key, k -> new LinkedList<>()).add(value);
    }
    ...
    public void forEach(BiConsumer<K, V> consumer) {
        for (Map.Entry<K, LinkedList<V>> entry : this.backingMap.entrySet()) {
    ...
    private class ValuesIterator implements Iterator<V> {
        ...
        private final Iterator<LinkedList<V>> listIterator;
    ...
    private class Values extends AbstractCollection<V> {
        ...
        @Override
        public int size() {
            return LinkedListMap.this.size();
        }
    }
}
