...
final class ListChangeBuilder<E> {
    ...
    private void insertUpdate(int pos) {
        ...
        if (idx < 0) { //If not found
            ...
            if (idx > 0 && (change = updateChanges.get(idx - 1)).to == pos) {
                ...
                updateChanges.add(idx, new SubChange<>(pos, pos + 1, null, EMPTY_PERM, true));
            }
    ...
    private void insertRemoved(int pos, final E removed) {
        ...
        if (idx < 0) { // Not found
            ...
            if (idx > 0 && (change = addRemoveChanges.get(idx - 1)).to == pos) {
                ...
                change.from--;
                change.to--;
                change.removed.addFirst(removed);
            } else {
    ...
    private void insertAdd(int from, int to) {
        ...
        if (idx < 0) { // Not found
            ...
            if (idx > 0 && (change = addRemoveChanges.get(idx - 1)).to == from) {
                ...
                addRemoveChanges.add(idx, new SubChange<>(from, to, new ArrayList<>(), EMPTY_PERM, false));
            }
        } else {
            SubChange<E> change = addRemoveChanges.get(idx);
    ...
    private int compress(List<SubChange<E>> list) {
        ...
        SubChange<E> prev = list.getFirst();
    ...
    public void nextRemove(int idx, E removed) {
        ...
        final SubChange<E> last = addRemoveChanges.isEmpty() ? null : addRemoveChanges.getLast();
    ...
    public void nextAdd(int from, int to) {
        ...
        if (updateChanges != null && !updateChanges.isEmpty()) {
            ...
            if (uPos < 0) {
                ...
                updateChanges.add(uPos + 1, new SubChange<>(to, change.to + to - from, null, EMPTY_PERM, true));
                change.to = from;
    ...
    public void nextPermutation(int from, int to, int[] perm) {
        ...
        if ((addRemoveChanges != null && !addRemoveChanges.isEmpty())) {
            ...
            int last = 0;
            int offset = 0;
            for (SubChange<E> change : addRemoveChanges) {
        ...
        if (updateChanges != null && !updateChanges.isEmpty()) {
            ...
            for (SubChange<E> change : updateChanges) {
                for (int cIndex = change.from; cIndex < change.to; ++cIndex) {
                    if (cIndex < from || cIndex >= to) {
    ...
    public void nextUpdate(int idx) {
        ...
        final SubChange<E> last = updateChanges.isEmpty() ? null : updateChanges.getLast();
        if (last != null && last.to == idx) {
            last.to = idx + 1;
        } else {
            insertUpdate(idx);
        ...
        if (changeLock == 0
            ...
            if (totalSize == 1) {
                if (addRemoveNotEmpty) {
                    list.fireChange(new SingleChange<>(finalizeSubChange(addRemoveChanges.getFirst()), list));
                    addRemoveChanges.clear();
                } else if (updateNotEmpty) {
                    list.fireChange(new SingleChange<>(finalizeSubChange(updateChanges.getFirst()), list));
                    updateChanges.clear();
                } else {
                ...
                if (addRemoveNotEmpty) {
                    for (final SubChange<E> change : addRemoveChanges) {
                        if (change != null) {
                            array[ptr++] = change;
                        }
                    }
                ...
                if (updateNotEmpty) {
                    for (final SubChange<E> change : updateChanges) {
                        if (change != null) {
                            array[ptr++] = change;
                        }
                    }
