...
import com.stardevllc.starlib.table.TreeTable;

import java.util.*;

public class ArrayListTreeTable<R extends Comparable<R>, C extends Comparable<C>, V> extends TreeTable<R, C, List<V>> implements ListTable<R, C, V> {
    
    @Override
    public int size() {
        int size = 0;
        
        for (Map.Entry<R, TreeMap<C, List<V>>> e1 : this.backingMap.entrySet()) {
    ...
    public void add(R row, C column, V value) {
        List<V> list = computeIfAbsent(row, column, (r, c) -> new ArrayList<>());
        list.add(value);
    }
    ...
    protected class Elements extends AbstractCollection<V> {
        ...
        @Override
        public int size() {
            return ArrayListTreeTable.this.size();
        }
    }
}
