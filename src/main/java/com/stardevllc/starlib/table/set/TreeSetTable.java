...
public class TreeSetTable<R extends Comparable<R>, C extends Comparable<C>, V> extends TreeTable<R, C, Set<V>> implements SetTable<R, C, V> {
    
    @Override
    public int size() {
        int size = 0;
        
        for (Map.Entry<R, TreeMap<C, Set<V>>> e1 : this.backingMap.entrySet()) {
    ...
    public void add(R row, C column, V value) {
        Set<V> list = computeIfAbsent(row, column, (r, c) -> new TreeSet<>());
        list.add(value);
    }
